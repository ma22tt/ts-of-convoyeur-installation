import type { Charge, Etat, Machine } from "./types.js";
...
export class Installation {
  ...
  #machine(id: number): Machine | undefined {
    return trouver(this.#machines, id);
  }

  #charge(id: number): Charge | undefined {
  ...
  deposer(machineId: number, chargeId: number): void {
    ...
    const charge: Charge = { id: chargeId, etat: "posee", machine: machineId };
  ...
  compter(): Record<Etat, number> {
    const total: Record<Etat, number> = { posee: 0, en_transit: 0, retiree: 0 };
